...
public class Local_Host extends base {
	...
	public void PatientModule() throws InterruptedException {
		...
		while (true) {
			try {
				WebElement $patient_btn$ = driver.findElement(By.xpath("//td[text()='Patient']"));
				visbility(driver, $patient_btn$, 60);
				j.executeScript("arguments[0].click();", $patient_btn$);
				break;
			} catch (Exception e) {
		...
		while (true) {
			try {
				WebElement $patientInfo$ = driver.findElement(By.xpath("//div[@id='patientInfo']/div[1]/span[2]"));
				visbility(driver, $patientInfo$, 60);
				javascriptclick($patientInfo$);
				break;
			} catch (Exception e) {
	...
	public void Settings() throws InterruptedException, IOException {
		...
		sendkeys(trp11, ConfigManager.getconfigManager().getInstanceConfigReader().getpass());
		WebElement trp12 = driver.findElement(By.id("confirmNewPassword"));
		visbility(driver, trp12, 60);
		sendkeys(trp12, ConfigManager.getconfigManager().getInstanceConfigReader().getpass());
